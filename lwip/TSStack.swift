...
public final class TSIPStack {
    ...
    public func receivedPacket(_ data: Data) {
        dispatch_call {
            ...
            data.copyBytes(to: buf.pointee.payload.bindMemory(to: UInt8.self, capacity: data.count), count: data.count)
            ...
            _ = netif_list.pointee.input(buf, self.interface)
        }
    }
    
    func writePBuf(_ buf: UnsafeMutablePointer<pbuf>) {
        var data = Data(count: Int(buf.pointee.tot_len))
        _ = data.withUnsafeMutableBytes {
            pbuf_copy_partial(buf, $0, buf.pointee.tot_len, 0)
        }
